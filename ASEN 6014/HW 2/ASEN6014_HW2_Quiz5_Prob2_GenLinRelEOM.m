...
[tout, state_out] = ode45(@(t, state)linearized_nonlinear_rel_eom(t, state), [0, time], state_0);
...
% Task 2
function state_dot = linearized_nonlinear_rel_eom(t, state)
    ...
    fdot = h/rc^2;
    x_dotdot = 2*fdot*(ydot - y*rc_dot/rc) + x*fdot^2*(1 + 2*rc/p);
    y_dotdot = -2*fdot*(xdot - x*rc_dot/rc) + y*fdot^2*(1 - rc/p);
    z_dotdot = -rc/p*fdot^2*z;
