...
sigma_ss = 1/K * delta_L;
% sigma_ss = [0.1, -0.06, 0.04 ]

state_0 = [sigma_BN_t0; omega_BN_B_t0];

[state, u] = rk4(@(state, t, u_B, I_B)diff_eq(state, t, u_B, I_B, delta_L), t_span, state_0, I_B, K, P);

sigma_BN_at_35 = sigma_BR(:, 7001);
sigma_BN_norm_at_35 = sqrt(sigma_BN_at_35(1)^2 + sigma_BN_at_35(2)^2 + sigma_BN_at_35(3)^2);

function state_dot = diff_eq(state, t, u, I, delta_L)
    % 6x1 state EOMs
    % Inputs:
    ...
    omega_BN_B_dot(1,1) = -(I(3,3) - I(2,2))/I(1,1) * omega_BN_B(2) * omega_BN_B(3) + u(1)/I(1,1) + delta_L(1)/I(1,1);
    omega_BN_B_dot(2,1) = -(I(1,1) - I(3,3))/I(2,2) * omega_BN_B(1) * omega_BN_B(3) + u(2)/I(2,2) + delta_L(2)/I(2,2);
    omega_BN_B_dot(3,1) = -(I(2,2) - I(1,1))/I(3,3) * omega_BN_B(2) * omega_BN_B(1) + u(3)/I(3,3) + delta_L(3)/I(3,3);
