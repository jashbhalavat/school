...
% ASEN 5010 - Project, Programming Assignment, Task 11
% Spring 2025
% Jash Bhalavat
...
[state_task11, u_task11, sigma_BR_task11, omega_BR_B_task11, sc_eclipsed, angle_difference, comm_window] = rk4_task11(@(state, t, u_B, I_B)diff_eq(state, t, u_B, I_B), t, state_0, I_B, K, P, r_N_lmo, r_dot_N_lmo, theta_dot_lmo, r_N_gmo, r_mars);
...
legend("$\sigma_{B/N}(1)$", "$\sigma_{B/N}(2)$", "$\sigma_{B/N}(3)$", "Eclipse", "Communication Window", 'FontSize', 12, Interpreter="latex")
...
xregion(t(eclipsed_time_idx(1)), t(eclipsed_time_idx(2)));
xregion(t(comm_window_time_idx(1)), t(comm_window_time_idx(2)), FaceColor='b');
legend("$^B\omega_{B/N}(1)$", "$^B\omega_{B/N}(2)$", "$^B\omega_{B/N}(3)$", "Eclipse", "Communication Window", 'FontSize', 12, Interpreter="latex")
...
xregion(t(eclipsed_time_idx(1)), t(eclipsed_time_idx(2)));
xregion(t(comm_window_time_idx(1)), t(comm_window_time_idx(2)), FaceColor='b');
legend("$\sigma_{B/R}(1)$", "$\sigma_{B/R}(2)$", "$\sigma_{B/R}(3)$", "Eclipse", "Communication Window", 'FontSize', 12, Interpreter="latex")
...
xregion(t(eclipsed_time_idx(1)), t(eclipsed_time_idx(2)));
xregion(t(comm_window_time_idx(1)), t(comm_window_time_idx(2)), FaceColor='b');
legend("$^B\omega_{B/R}(1)$", "$^B\omega_{B/R}(2)$", "$^B\omega_{B/R}(3)$", "Eclipse", "Communication Window", 'FontSize', 12, Interpreter="latex")
...
function [x, u_B, sigma_BR, omega_BR_B, sc_eclipsed, angle_difference, comm_window] = rk4_task11(fn, t, x0, I_B, K, P, r_N_lmo, r_dot_N_lmo, theta_dot_lmo, r_N_gmo, r_mars)
    ...
    for i = 1:length(t)
        ...
        if sc_eclipsed(i) == false
            ...
            if comm_window(i)
                [DCM_RN, omega_RN_N] = gmo_reference_frame(r_N_lmo(i+1,:)', r_N_gmo(i+1,:)', t(i+1), r_N_lmo(i,:)', r_N_gmo(i,:)', t(i));
            else
                [DCM_RN, omega_RN_N] = nadir_reference_frame(r_N_lmo(i,:)', r_dot_N_lmo(i,:)', theta_dot_lmo);
            end
        end
        [sigma_BR(i,:), omega_BR_B(i,:)] = attitude_error(t(i), sigma_BN, omega_BN_B, DCM_RN, omega_RN_N);
