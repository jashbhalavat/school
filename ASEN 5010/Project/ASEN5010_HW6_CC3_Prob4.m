...
[state, u, sigma_BR] = rk4(@(state, t, u_B, I_B)diff_eq(state, t, u_B, I_B, L, delta_L), t_span, state_0, I_B, K, P, f, L);

sigma_BR_at_35 = sigma_BR(:, 3501);
sigma_BR_norm_at_35 = sqrt(sigma_BR_at_35(1)^2 + sigma_BR_at_35(2)^2 + sigma_BR_at_35(3)^2);

function state_dot = diff_eq(state, t, u, I, L, delta_L)
    % 6x1 state EOMs
    % Inputs:
    ...
    omega_BN_B_dot(1,1) = -(I(3,3) - I(2,2))/I(1,1) * omega_BN_B(2) * omega_BN_B(3) + u(1)/I(1,1) + L(1)/I(1,1) + delta_L(1)/I(1,1);
    omega_BN_B_dot(2,1) = -(I(1,1) - I(3,3))/I(2,2) * omega_BN_B(1) * omega_BN_B(3) + u(2)/I(2,2) + L(2)/I(2,2) + delta_L(2)/I(2,2);
    omega_BN_B_dot(3,1) = -(I(2,2) - I(1,1))/I(3,3) * omega_BN_B(2) * omega_BN_B(1) + u(3)/I(3,3) + L(3)/I(3,3) + delta_L(3)/I(3,3);
...
function [x, u_B, sigma_BR] = rk4(fn, t, x0, I_B, K, P, f, L)
    ...
    for i = 1:length(t)-2
        ...
        sigma_RN_ip1 = zeros([3,1]);
        RN_ip1 = mrp_to_dcm(sigma_RN_ip1);

        sigma_RN_ip2 = zeros([3,1]);
        ...
        u_B(i,:) = (-K*sigma_BR(:,i) -P*omega_BR_B + I_B*(omega_dot_RN_B - cross(omega_BN_B, omega_RN_B)) + skew_symmetric(omega_BN_B)*I_B*omega_BN_B - L)';
