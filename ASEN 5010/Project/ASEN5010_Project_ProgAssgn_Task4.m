...
% ASEN 5010 - Project, Programming Assignment, Task 4
% Spring 2025
% Jash Bhalavat
...
DCM_RsN_at_0 = sun_reference_frame_orientation;

function DCM_RsN = sun_reference_frame_orientation()
    r1_N = [-1, 0, 0]';
    r3_N = [0, 1, 0]';
...
for i = 1:length(t_lmo)
    [DCM_RnN(:,:,i), omega_RnN_N(:,i)] = nadir_reference_frame_orientation(r_N_lmo(i,:)', r_dot_N_lmo(i,:)', theta_dot_lmo);
end
...
figure()
plot(t_lmo, omega_RnN_N, 'LineWidth',2)
legend("$^N\omega_{Rn/N}(1)$", "$^N\omega_{Rn/N}(2)$", "$^N\omega_{Rn/N}(3)$", 'FontSize', 12, Interpreter="latex")
ylabel("$^N\omega_{Rn/N}$ [rad/sec]", 'FontSize', 14, Interpreter="latex")
xlabel("Time [sec]")
grid on
title("Angular Velocity of Nadir Pointing Reference w.r.t. Inertial Frame in Inertial Frame")
% DCM_RnN_at_330 = nadir_reference_frame_orientation;

function [DCM_RnN, omega_RnN_N] = nadir_reference_frame_orientation(r_N, v_N, theta_dot)
    r1_N = -r_N/norm(r_N);
    r2_N = v_N/norm(v_N);
