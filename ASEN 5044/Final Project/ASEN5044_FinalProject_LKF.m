...
for ss = 1:num_sims
    ...
    [xlkf_out, Plkf_out, NEESsshist, NISsshist] = lkf(tspan, xout, F_lkf, perturb_x0, Rlkf, dy_lkf, H_lkf, P_plus_0, Qlkf, omega, satID_nom, xk_truehist, ykhist);
    ...
    figure()
    subplot(2,2,1)
    plot(tspan, xlkf_out(1,:), tspan, xk_truehist(1,:))
    xlabel("Time [sec]")
    ylabel("X [m]")
    legend("LKF Output", "True trajectory")

    subplot(2,2,2)
    plot(tspan, xlkf_out(2,:), tspan, xk_truehist(2,:))
    xlabel("Time [sec]")
    ylabel("Xdot [m/s]")

    subplot(2,2,3)
    plot(tspan, xlkf_out(3,:), tspan, xk_truehist(3,:))
    xlabel("Time [sec]")
    ylabel("Y [m]")

    subplot(2,2,4)
    plot(tspan, xlkf_out(4,:), tspan, xk_truehist(4,:))
    xlabel("Time [sec]")
    ylabel("Ydot [m/s]")
    sgtitle("Satellite Estimated LKF vs True state for a single sim run")

    figure()
    subplot(2,2,1)
    plot(tspan, xlkf_out(1,:) - xk_truehist(1,:))
    % hold on
    % plot(tspan, sqrt(squeeze(Plkf_out(1,1,:)))*2)
    xlabel("Time [sec]")
    ylabel("X [m]")

    subplot(2,2,2)
    plot(tspan, xlkf_out(2,:) - xk_truehist(2,:))
    xlabel("Time [sec]")
    ylabel("Xdot [m/s]")

    subplot(2,2,3)
    plot(tspan, xlkf_out(3,:) - xk_truehist(3,:))
    xlabel("Time [sec]")
    ylabel("Y [m]")

    subplot(2,2,4)
    plot(tspan, xlkf_out(4,:) - xk_truehist(4,:))
    xlabel("Time [sec]")
    ylabel("Ydot [m/s]")
    sgtitle("Satellite Estimated Errors")
...
function [xhat_plus, P_plus, NEESsshist, NISsshist] = lkf(tvec, xstar, F, dxhat_plus_0, R_in, dy, H, P_plus_0, Q, omega, satID, xk_truehist, yk_hist)
    I = eye(4);
    ...
    P_plus(:,:,1) = P_plus_0;
    invPkp1 = inv(P_plus(:,:,1));
    NEESsshist(1) = ...
            (xk_truehist(:,1) - xhat_plus(:,1))'*invPkp1*(xk_truehist(:,1) - xhat_plus(:,1));
    ...
    for i = 2:length(tvec)
        ...
        dy_at_time = [];
        for station = 1:12
            if ~isnan(satID(i,station))
                R((3*count-2):(3*count),(3*count-2):(3*count)) = R_in;
                H_at_time(3*count-2:3*count, :) = H(3*station-2:3*station,:,i);
                dy_at_time(3*count-2:3*count,:) = dy(3*station-2:3*station,i);
                count = count + 1;
            end
        end
        % If no measurements!
        if count == 1
            % Setting innovation matrix and Pyy to a big number because no
            % meausrement is available
            Pyy_kp1 = ones(3);
            innov = [1; 1; 1]; 
            dxhat_plus(:,i) = dxhat_minus(:,i);
            P_plus(:,:,i) = P_minus(:,:,i);
        else
            Pyy_kp1 = H_at_time*P_minus(:,:,i)*H_at_time' + R;
            K = P_minus(:,:,i)*H_at_time'*inv(Pyy_kp1);
            innov = dy_at_time - H_at_time*dxhat_minus(:,i);
            dxhat_plus(:,i) = dxhat_minus(:,i) + K*innov;
            P_plus(:,:,i) = (I - K*H_at_time)*P_minus(:,:,i);
        end
        % Adding up deterministic and perturbations
        xhat_plus(:,i) = xstar(:,i) + dxhat_plus(:,i);

        invPkp1 = inv(P_plus(:,:,i));
        invPyykp1 = inv(Pyy_kp1);
        NEESsshist(i) = ...
            (xk_truehist(:,i) - xhat_plus(:,i))'*invPkp1*(xk_truehist(:,i) - xhat_plus(:,i));
        NISsshist(i) = innov'*invPyykp1*innov;
    end
end
