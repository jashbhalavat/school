...
function out = A_func(xnom, mu)
    out = [0 1 0 0; -mu*(xnom(1,1)^2+xnom(3,1)^2)^(-3/2) + 3*mu*xnom(1,1)^2*(xnom(1,1)^2+xnom(3,1)^2)^(-5/2) 0 3*mu*xnom(1,1)*xnom(3,1)*(xnom(1,1)^2+xnom(3,1)^2)^(-5/2) 0; ...
...
for ss = 1:num_sims
    ...
    ykhist = NaN(p,stations,length(tspan));
    
    wk = mvnrnd(zeros(4,1),Q4x4)';
    xk_truehist(:,1) = x0-perturb_x0 + wk;
    ...
    for jj = 2:length(tspan)
        ...
        xk_truehist(:,jj) = ode45(eom_full_nl(xk_truehist(:,jj-1),mu,wk),[ti, tf],xk_truehist(:,jj-1),options);
        vk = mvnrn(zeros(p,1),Rtrue);
        for station = 1:stations
...
function [xhat_plus, P_plus, dy] = lkf(tvec, xstar, F, dxhat_plus, R, y, H)
    for i = 2:length(tvec)
        % Time Update
        dxhat_minus(:,i) = F(:,:,i-1)*dxhat_plus(:,i-1);
        P_minus(:,:,i) = F(:,:,i-1)*P_plus(:,:,i-1)*F(:,:,i-1)' + omega(:,:,i-1)*Q(:,:,i-1)*omega(:,:,i-1);

        % Measurement Update
        K(:,:,i) = P_minus(:,:,i)*H(:,:,i)'*inv(H(:,:,i)*P_minus(:,:,i)*H(:,:,i)' + R(:,:,i));
        dy(:,i) = y(:,i) - h(xstar(:,i));
        dxhat_plus(:,i) = dxhat_minus(:,i) + K(:,:,i)*(dy(:,i) - H(:,:,i)*dxhat_minus(:,i));
