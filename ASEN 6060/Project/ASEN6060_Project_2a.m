...
grid on
axis equal
title("2A Initial & Final Periodic Orbits")
...
for k = 1:length(L1_manifolds)
    count = 0;
    counted_once = false;
    [moon_unstable_t, moon_unstable_x] = ode113(@(t, state)CR3BP(state, mu), [0, L1_manifold_time], L1_manifolds(:,k), options);
...
for k = 1:length(L2_manifolds)
    count = 0;
    counted_once = false;
    [moon_stable_t, moon_stable_x] = ode113(@(t, state)CR3BP(state, mu), [0, -L2_manifold_time], L2_manifolds(:,k), options);
...
function moon_bound_unstable = part_b(tout, xout, mu, l1_pos, manifold_time, n_crossings)
    % Set options for ode113()
    % Part b
...
function moon_bound_stable = part_c(tout, xout, mu, l2_pos, manifold_time, n_crossings)
    ...
    for i = 1:length(tout)
        ...
        %     end
        % end
        [~, stable_index] = min(real(evals_minus_1));
