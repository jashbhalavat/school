...
% Trying to manually set V0 s.t. C = 3.17, but that doesn't work
% C = u_star_times_2(V0_1(1), V0_1(2), mu) - V0_1(5)^2 - V0_1(4)^2;
% r1 = sqrt((x0_1(1)+mu)^2);
% r2 = sqrt((x0_1(1)-1+mu)^2);
% C_des = 3.17;
% y_dot = sqrt(x0_1(1)^2 + 2*(1-mu)/r1 + 2*mu/r2 - C_des);
% V0_1(5) = -y_dot;
% C = u_star_times_2(V0_1(1), V0_1(2), mu) - V0_1(5)^2 - V0_1(4)^2;

L1_periodic = gen_3d_periodic_orbit_single_shooting(V0_1, mu, false);

[L1_tout, L1_xout] = ode113(@(t, state)CR3BP_full(state, mu), [0, L1_periodic(end)], [L1_periodic(1:6); reshape(eye(6), [36,1])], options);

% C = x^2 + y^2 + 2*(1-mu)/r1 + 2*mu/r2 - (x_dot^2 + y_dot^2 + z_dot^2);
...
for i = 1:length(L1_tout)
    C(i) = u_star_times_2(L1_xout(i,1), L1_xout(i,2), mu) - L1_xout(i,5)^2 - L1_xout(i,4)^2;
end
...
plot(L1_xout(:,1), L1_xout(:,2), 'LineWidth',2)
hold off
legend("L1", "Moon")
...
title("Moon-Bound Unstable Manifold associated with L1 Lyapunov Orbit")
%%

figure()
scatter(poincare_stored(:,2), poincare_stored(:,1), 10, 'filled', 'red');
