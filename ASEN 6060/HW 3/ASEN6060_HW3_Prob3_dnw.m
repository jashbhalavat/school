...
for i = 1:family_members
    ...
    [t_out, state_out] = ode113(@(t,state)CR3BP_full(state, mu), [0, V(7,i)], state0, options);
    ...
    H_i = H(V_soln, xout_corrected, delta);
    DH_V = DH_mat(statef, state_dot);
    V(:,i+1) = Vi - inv(DH_V) * H_i;
...
% 
% plot(xout_corrected(:,1), xout_corrected(:,2), 'LineWidth',2)
% legend("CR3BP Propagation", "L1", "Initial Variation", "Corrected Trajectory")
