...
% Given
% x0 = [0.82340, 0, -0.026755/10,0,0.13742,0]';
% x0 = [ 0.836991461, 0.0, 0.002601456816, 0.0, 0.00018021945, 0.0 ]';
x0 = [0.836395, 0.0, 0.002601, 0.0, 0.01000, 0.0]';
% x0 = [0.841466039279, 0.0, 0.002601456816, 0.0, -1.017500081771e-02, 0.0]';
% x0 = [0.841466039279, 0.0, 0.026014568158, 0.0, -1.017500081771e-02, 0.0]';
% T = 2.7477;
T = 2.6613640381255497;
% T = 2.004751450968;
% T = 2.004751450968;
V0 = [x0; T];
...
[tout, xout] = ode113(@(t,state)CR3BP(state, mu), [0, V0(7,1)], V0(1:6,1), options);
plot3(xout(:,1), xout(:,2), xout(:,3), 'LineWidth',2)
hold on 
[tout, xout] = ode113(@(t,state)CR3BP(state, mu), [0, V_soln(7,1)], V_soln(1:6,1), options);
plot3(xout(:,1), xout(:,2), xout(:,3), 'LineWidth',2)
...
% save("V_family_halo_L1.mat", "V_family");
% load("V_family_halo_L1.mat");
